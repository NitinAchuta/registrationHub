...
import { getNormalizedCompanies, type NormalizedCompany, type RegistrationEntry } from "@/lib/data"

type SortField = "canonicalName" | "industry" | "topMajor" | "package" | "status"
type SortDir = "asc" | "desc"

const packageOrder = ["Platinum", "Gold", "Silver", "Bronze"]
const statusOptions = ["Confirmed", "Pending", "Waitlisted", "Cancelled"]
const majorGroups = [
  "Computer Science",
...
function mapCSVToEntries(headers: string[], rows: string[][], mapping: Record<string, string>): Partial<RegistrationEntry>[] {
...
function SortIcon({ field, sortField, sortDir }: { field: SortField; sortField: SortField; sortDir: SortDir }) {
  if (field !== sortField) return <ChevronsUpDown className="h-3 w-3 text-muted-foreground" />
...
function CompanySheet({ company, open, onClose }: { company: NormalizedCompany | null; open: boolean; onClose: () => void }) {
      ...
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        <SheetHeader className="pb-4 border-b border-border">
          <div className="flex items-start justify-between gap-2">
            <div>
              <SheetTitle className="text-lg font-semibold text-foreground">{company.canonicalName}</SheetTitle>
              <SheetDescription className="text-sm text-muted-foreground mt-0.5">{company.industry}</SheetDescription>
            </div>
          ...
          {company.variants.length > 1 && (
            <div className="flex items-center gap-1.5 mt-2 p-2 rounded-md bg-amber-50 border border-amber-200">
              ...
              <p className="text-xs text-amber-700">
                <span className="font-medium">Normalized from {company.variants.length} entries:</span>{" "}
        ...
        <div className="px-1 pb-8">
          ...
          <DetailRow label="Package" value={<Badge variant="outline" className={packageColor[primary.package]}>{primary.package}</Badge>} />
          <DetailRow label="Virtual Fair" value={primary.virtualFair ? "Yes" : "No"} />
          <DetailRow
            label="Wi-Fi"
            value={
          ...
          <DetailRow label="Days Attending" value={primary.daysAttending.join(", ")} />

          <SectionHeader icon={GraduationCap} title="Recruitment" />
          <DetailRow label="Top Major" value={primary.topMajor} />
          <DetailRow label="Majors Recruited" value={primary.majorsRecruited.join(", ")} />
          <DetailRow label="Degree Levels" value={primary.degreeLevels.join(", ")} />
          <DetailRow label="Position Types" value={primary.positionTypes.join(", ")} />
          ...
          <DetailRow
            ...
            value={
              <span className="flex items-center gap-1 justify-end">
                ...
                <a href={`mailto:${primary.repEmail}`} className="text-primary hover:underline">{primary.repEmail}</a>
              </span>
            }
          />
          <DetailRow
          ...
          />
          <DetailRow label="Add&apos;l Reps Day 1" value={primary.additionalRepsDay1 || "—"} />
          <DetailRow label="Add&apos;l Reps Day 2" value={primary.additionalRepsDay2 || "—"} />

          {company.entries.length > 1 && (
            <>
              <div className="flex items-center gap-2 pt-4 pb-2">
                <Calendar className="h-4 w-4 text-primary" />
                <h3 className="text-sm font-semibold text-foreground">All Registrations ({company.entries.length})</h3>
              </div>
              <div className="space-y-2">
                {company.entries.map((e) => (
                  <div key={e.id} className="rounded-md border border-border p-3 text-sm">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-medium text-foreground">{e.organization}</span>
                      <Badge variant="outline" className={`text-xs ${packageColor[e.package]}`}>{e.package}</Badge>
                    </div>
                    <span className="text-xs text-muted-foreground">{e.repName} · {e.repEmail}</span>
                  </div>
                ))}
              </div>
            </>
...
function CSVImportDialog({
  ...
  const crmFields = ["organization", "package", "status", "repName", "repEmail", "repPhone", "topMajor", "industry", "boothLocation"]
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "upload" && "Upload a CSV file to import company registrations."}
            ...
            {csvData.headers.map((header) => (
              <div key={header} className="flex items-center justify-between gap-4">
                ...
                <Select
                  ...
                  onValueChange={(value) => setMapping((m) => ({ ...m, [header]: value === "skip" ? "" : value }))}
                >
        ...
        {step === "success" && (
          <div className="py-8 text-center">
            ...
            <p className="text-lg font-semibold text-foreground mb-1">Import Successful</p>
            <p className="text-sm text-muted-foreground">
              {importedCount} {importedCount === 1 ? "record" : "records"} imported and added to the table.
            </p>
          </div>
        )}
        ...
        <DialogFooter>
          ...
          {step === "mapping" && (
            <>
              ...
              <Button onClick={handleImport} disabled={!Object.values(mapping).some(v => v === "organization")}>
...
export function CompanyTable() {
  ...
  const [importedEntries, setImportedEntries] = useState<Partial<RegistrationEntry>[]>([])

  const baseCompanies = useMemo(() => getNormalizedCompanies(), [])

  const companies = useMemo(() => {
    if (importedEntries.length === 0) return baseCompanies

    const importedCompanies: NormalizedCompany[] = importedEntries.map((entry, i) => ({
      ...
      entries: [{
        ...
        status: (entry.status as RegistrationEntry["status"]) || "Pending",
    ...
    return [...baseCompanies, ...importedCompanies]
  }, [baseCompanies, importedEntries])

  const activeFilterCount = filters.packages.length + filters.statuses.length + filters.majors.length
  ...
  const filtered = useMemo(() => {
    ...
    return companies.filter((c) => {
      ...
      const matchesMajor =
        ...
        filters.majors.some((major) =>
          c.topMajor.toLowerCase().includes(major.toLowerCase()) ||
          c.entries.some((e) =>
            e.majorsRecruited.some((m) => m.toLowerCase().includes(major.toLowerCase()))
          )
        )
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Popover open={filterOpen} onOpenChange={setFilterOpen}>
          ...
          <PopoverContent className="w-80" align="end">
            <div className="space-y-4">
              ...
              <div>
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Package Type</p>
              ...
              <div>
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Major Group</p>
                <div className="space-y-2">
                  {majorGroups.map((major) => (
                    <div key={major} className="flex items-center gap-2">
                      ...
                      />
                      <Label htmlFor={`major-${major}`} className="text-sm cursor-pointer">
                        {major}
                      </Label>
                    </div>
                  ))}
        ...
        <Button variant="outline" className="gap-2 shrink-0" onClick={() => setCsvDialogOpen(true)}>
        ...
        <div className="flex flex-wrap gap-2">
          {filters.packages.map((pkg) => (
            <Badge key={pkg} variant="secondary" className="gap-1 pr-1">
              {pkg}
              <button onClick={() => toggleFilter("packages", pkg)} className="ml-1 hover:bg-muted rounded p-0.5">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
          {filters.statuses.map((status) => (
            <Badge key={status} variant="secondary" className="gap-1 pr-1">
              {status}
              <button onClick={() => toggleFilter("statuses", status)} className="ml-1 hover:bg-muted rounded p-0.5">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
          {filters.majors.map((major) => (
            <Badge key={major} variant="secondary" className="gap-1 pr-1">
              {major}
              <button onClick={() => toggleFilter("majors", major)} className="ml-1 hover:bg-muted rounded p-0.5">
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
            ...
            <tbody>
              {sorted.map((company, i) => (
                <tr
                  ...
                  className={`border-b border-border last:border-0 cursor-pointer transition-colors hover:bg-accent/50 ${i % 2 === 0 ? "bg-background" : "bg-muted/20"}`}
                  onClick={() => handleRowClick(company)}
                >
                  <td className="px-4 py-3">
                    <div>
                      <span className="font-medium text-foreground">{company.canonicalName}</span>
                      {company.variants.length > 1 && (
                        <div className="flex items-center gap-1 mt-0.5">
                          <AlertCircle className="h-3 w-3 text-amber-500" />
                          <span className="text-xs text-amber-600">{company.variants.length} variants merged</span>
                        </div>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">{company.industry}</td>
                  <td className="px-4 py-3 text-muted-foreground">{company.topMajor}</td>
                  <td className="px-4 py-3">
                    <Badge variant="outline" className={`text-xs ${packageColor[company.package]}`}>
                      {company.package}
                    </Badge>
                  </td>
                  <td className="px-4 py-3">
                    <Badge variant="outline" className={`text-xs ${statusColor[company.status]}`}>
                      {company.status}
                    </Badge>
                  </td>
                </tr>
      ...
      <p className="text-xs text-muted-foreground">
        {sorted.length} of {companies.length} organizations &middot; {companies.reduce((s, c) => s + c.entries.length, 0)} total registrations
        {importedEntries.length > 0 && ` &middot; ${importedEntries.length} imported`}
      </p>

      <CompanySheet company={selected} open={sheetOpen} onClose={() => setSheetOpen(false)} />
      <CSVImportDialog open={csvDialogOpen} onClose={() => setCsvDialogOpen(false)} onImport={handleCSVImport} />
    </div>
  )
}
